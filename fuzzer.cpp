...
void Fuzzer::ParseOptions(int argc, char **argv)
{
    ...
    if ((in_dir == "-") || GetBinaryOption("-restore", argc, argv, false) || GetBinaryOption("-resume", argc, argv, false))
    {
...
void Fuzzer::Run(int argc, char **argv)
{
    ...
    else
    {
        ...
        //设置第一步状态为INPUT_SAMPLE_PROCESSING
...
void Fuzzer::SynchronizeAndGetJob(ThreadContext *tc, FuzzerJob *job)
{
    ...
    if (state == FUZZING)
    {
        ...
        if ((cur_time > last_save_time) && (((cur_time - last_save_time) / 1000) > FUZZER_SAVE_INERVAL))
        {
            SaveState(tc);
    ...
    if ((state == FUZZING) && server && (GetCurTime() > (last_server_update_time_ms + server_update_interval_ms)))
    {
    ...
    {
        //fuzz刚开始状态为INPUT_SAMPLE_PROCESSING
...
void Fuzzer::FuzzJob(ThreadContext *tc, FuzzerJob *job)
{
    ...
    while (1)
    {
        ...
        if ((entry->num_hangs > 10) && (entry->num_hangs > (entry->num_runs * acceptable_hang_ratio)))
        {
            WARN("Sample %lld produces too many hangs. Discarding\n", entry->sample_index);
            job->discard_sample = true;
            break;
        }
        if ((entry->num_crashes > 100) && (entry->num_crashes > (entry->num_runs * acceptable_crash_ratio)))
        {
