...
template <typename T>
void Mutator::AddDefaultInterestingValues(std::vector<Sample> &interesting_values)
{
...
void InterestingValueMutator::DictUnescape(std::string &in, std::string &out)
{
    ...
    {
        if ((in_buf[in_pos] == '\\') && (in_buf[in_pos + 1] == 'x') && isxdigit(in_buf[in_pos + 2]) && isxdigit(in_buf[in_pos + 3]))
        {
